...
void Laborator7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 &modelMatrix, const glm::vec3 &color) {
    ...
    glm::vec3 vecEyePos = GetSceneCamera()->transform->GetWorldPosition();

    GLint locEyePos = glGetUniformLocation(shader->program, "eye_position");
    glUniform3fv(locEyePos, 1, glm::value_ptr(vecEyePos));

    // TODO: Set material property uniforms (shininess, kd, ks, object color)
    GLint locShine = glGetUniformLocation(shader->program, "material_shininess");
    glUniform1i(locShine, materialShininess);

    GLint locKd = glGetUniformLocation(shader->program, "material_kd");
    glUniform1f(locKd, materialKd);

    GLint locKs = glGetUniformLocation(shader->program, "material_ks");
    glUniform1f(locKs, materialKs);

    GLint locObjColor = glGetUniformLocation(shader->program, "object_color");
    glUniform3fv(locObjColor, 1, glm::value_ptr(color));
