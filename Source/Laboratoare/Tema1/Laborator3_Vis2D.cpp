...
void Laborator3_Vis2D::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (window->KeyHold(GLFW_KEY_W)) {
		logicSpace.y -= deltaTime * moveVal;
	}
	...
	if (window->KeyHold(GLFW_KEY_D)) {
		logicSpace.x -= deltaTime * moveVal;
	}
	...
	if (window->KeyHold(GLFW_KEY_X)) {
		logicSpace.x += logicSpace.width * 0.5;
		logicSpace.y += logicSpace.height * 0.5;
		
		logicSpace.width += deltaTime * zoomVal;
		logicSpace.height += deltaTime * zoomVal;
