...
class Camera {
   public:
    ...
    Camera(const glm::vec3 &position, const glm::vec3 &center, const glm::vec3 &up) {
        setFirtPersone(position, center, up);
    }

    ~Camera() {}

    // Update camera
    void setFirtPersone(const glm::vec3 &position, const glm::vec3 &center, const glm::vec3 &up) {
    ...
    void setThirdPersone(const glm::vec3 &position, const glm::vec3 &center, const glm::vec3 &up, const glm::vec3 &characterPos) {
        this->position = position;
        forward = glm::normalize(center - position);
        right = glm::cross(forward, up);
        this->up = glm::cross(right, forward);

        glm::vec3 aux = position - characterPos;
        distanceToTarget = sqrt(aux.x * aux.x + aux.y * aux.y + aux.z * aux.z);;
    }
    ...
    void RotateFirstPerson_OZ(float angle) {
        ...
        forward = glm::normalize(glm::vec3(glm::rotate(glm::mat4(1), angle, forward) * glm::vec4(up, 0)));
        up = glm::normalize(glm::cross(right, forward));
    }
