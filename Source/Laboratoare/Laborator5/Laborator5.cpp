...
void Laborator5::Init() {
    ...
    camera->Set(glm::vec3(0, 7, -7), glm::vec3(0, 1, 0), glm::vec3(0, 1, 0));
...
void Laborator5::Update(float deltaTimeSeconds) {
...

    {
        ...
        modelMatrix = glm::rotate(modelMatrix, RADIANS(75), glm::vec3(1, 0.5, 0));
        modelMatrix = glm::scale(modelMatrix, glm::vec3(0.75, 1, 0.5));
        RenderMesh(meshes["box"], shaders["Simple"], modelMatrix);
    }

    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, glm::vec3(-3, 2, -3));
        modelMatrix = glm::rotate(modelMatrix, RADIANS(120), glm::vec3(0.9, 0.3, 0.5));
...
void Laborator5::OnInputUpdate(float deltaTime, int mods) {
    ...
    if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT)) {
        ...
        if (window->KeyHold(GLFW_KEY_F)) {
            ...
            if (myProjection) {
                projectionMatrix = glm::perspective(RADIANS(fov), window->props.aspectRatio, nearVal, farVal);
            }
        }
...
void Laborator5::OnKeyPress(int key, int mods) {
    ...
    if (key == GLFW_KEY_O) {
        myProjection = false;
        projectionMatrix = glm::ortho(lft, rgt, bot, top, nearVal, farVal);
    }
